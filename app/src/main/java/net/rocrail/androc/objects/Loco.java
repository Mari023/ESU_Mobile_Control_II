...
public class Loco extends MobileImpl implements Runnable {
    ...
    */
        ...
        if (!this.rocrailService.Prefs.ImagesOnDemand) new Thread(this).start();
    ...
    public void addConsistMember(String memberID) {
        if (!Consist.contains(memberID)) {
            if (!Consist.isEmpty()) Consist = Consist + "," + memberID;
            else Consist = memberID;
            rocrailService.sendMessage("model", String.format("<model cmd=\"modify\"><lc id=\"%s\" consist=\"%s\"/></model>", ID, Consist));
        }
    }
    ...
    public void removeConsistMember(String memberID) {
        if (Consist.contains(memberID)) {
            ...
            if (Consist.contains(memberID)) Consist = Consist.replace(memberID + ",", "");
            if (Consist.contains(memberID)) Consist = Consist.replace(memberID, "");
            rocrailService.sendMessage("model", String.format("<model cmd=\"modify\"><lc id=\"%s\" consist=\"%s\"/></model>", ID, Consist));
        }
    }
    ...
    public void flipGo() {
        if (rocrailService.AutoMode) {
            AutoStart = !AutoStart;
            rocrailService.sendMessage("lc", String.format("<lc id=\"%s\" cmd=\"%s\"/>", ID, (AutoStart ? "go" : "stop")));
        }
    }

    public void doRelease() {
        rocrailService.sendMessage("lc", String.format("<lc throttleid=\"%s\" cmd=\"release\" id=\"%s\"/>", rocrailService.getDeviceName(), ID));
    }

    public void CVWrite(int cv, int val) {
        boolean longAddr = Addr > 127;
        rocrailService.sendMessage("program", String.format("<program cmd=\"1\" addr=\"%d\" decaddr=\"%d\" cv=\"%d\" value=\"%d\" longaddr=\"%s\" pom=\"true\"/>", Addr, Addr, cv, val, longAddr ? "true" : "false"));
    }

    public void CVRead(int cv) {
        boolean longAddr = Addr > 127;
        rocrailService.sendMessage("program", String.format("<program cmd=\"0\" addr=\"%d\" cv=\"%d\" longaddr=\"%s\" pom=\"true\"/>", Addr, cv, longAddr ? "true" : "false"));
    }

    public void setVmin(int v) {
        Vmin = v;
        rocrailService.sendMessage("model", String.format("<model cmd=\"modify\"><lc id=\"%s\" V_min=\"%d\"/></model>", ID, v));
    }

    public void setVmid(int v) {
        Vmid = v;
        rocrailService.sendMessage("model", String.format("<model cmd=\"modify\"><lc id=\"%s\" V_mid=\"%d\"/></model>", ID, v));
    }

    public void setVmax(int v) {
        Vmax = v;
        rocrailService.sendMessage("model", String.format("<model cmd=\"modify\"><lc id=\"%s\" V_max=\"%d\"/></model>", ID, v));
    }

    public void swap() {
        rocrailService.sendMessage("lc", String.format("<lc id=\"%s\" cmd=\"swap\"/>", ID));
    }

    public void Dispatch() {
        rocrailService.sendMessage("lc", String.format("<lc id=\"%s\" cmd=\"dispatch\"/>", ID));
    }
