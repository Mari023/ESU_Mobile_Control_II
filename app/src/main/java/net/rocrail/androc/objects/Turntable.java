...
public class Turntable extends Item {
    public List<TTTrack> Tracks = new ArrayList<>();
    public HashMap<String, TTTrack> TrackMap = new HashMap<>();
    ...
    public void updateWithAttributes(Attributes atts) {
        ...
        if (Tracks != null) {
            for (TTTrack track : Tracks) {
                track.State = (track.Nr == Bridgepos);
            }
        }

        super.updateWithAttributes(atts);
        if (imageView != null) imageView.post(new UpdateTT(this));
    }
    ...
    public void addTrack(Attributes atts) {
        ...
        Tracks.add(track);
        if (track.Description != null && !track.Description.isEmpty())
    ...
    void drawTraverser(Canvas canvas) {
        ...
        if (Sensor1 && Sensor2) paint.setColor(Color.RED);
    ...
    public void Draw(Canvas canvas) {
        ...
        for (TTTrack track : Tracks) {
            ...
            if (track.Show) canvas.drawLine((int) c79, (int) c79, x, y, paint);
        }
    ...
    public void OpenClose() {
        Closed = !Closed;
        m_RocrailService.sendMessage("tt", String.format("<tt id=\"%s\" state=\"%s\"/>", ID, Closed ? "closed" : "open"));
    }
...
class UpdateTT implements Runnable {
    ...
    @Override
    public void run() {
        if (item.imageView != null) item.imageView.invalidate();
    }
}
